...
def get_by_course(user_id, course_id):
    sql = "select count(answers.id) as count, answers.correct, answers.course_id, answers.user_id, courses.subject from answers join courses on courses.id = answers.course_id group by answers.correct, answers.course_id, answers.user_id, courses.subject having answers.course_id = :course_id and answers.user_id = :user_id"
...
def add(user_id, question_id, course_id, is_correct):
    try:
        ...
        db.session.execute(sql, {"user_id":user_id, "question_id":question_id, "course_id":course_id, "is_correct":is_correct})
        db.session.commit()
    except:
        return  False
    return True
